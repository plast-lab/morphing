...
public class FloatingPointLiteral extends Literal implements Cloneable {
 ...
 public void typeCheck() {
   ...
   if(constant().floatValue() == Float.NEGATIVE_INFINITY || constant().floatValue() == Float.POSITIVE_INFINITY)
    ...
    // Declared in CodeGeneration.jrag at line 513


  public void emitPushConstant(CodeGeneration gen) {
    if(constant().floatValue() == 0) gen.emit(Bytecode.FCONST_0);
    else if(constant().floatValue() == 1) gen.emit(Bytecode.FCONST_1);
    else if(constant().floatValue() == 2) gen.emit(Bytecode.FCONST_2);
    else {
  ...
  public boolean mayHaveRewrite() { return false; }
    ...
    // Declared in ConstantExpression.jrag at line 249
